...
class items_model extends CI_Model {
    ...
    function select_component() {
		...
		$select_component[''] = '--';
        $query = $this->db->query('SELECT cmp.cmp_id, cmp.cmp_code FROM '.$this->db->dbprefix('cmp').' AS cmp WHERE 1 GROUP BY cmp.cmp_id ORDER BY cmp.cmp_code');
    ...
    function select_section() {
		...
		$select_section[''] = '--';
        $query = $this->db->query('SELECT sct.sct_id, CONCAT(sct_trl.sct_trl_title, \' (\', sct.sct_code, \')\') AS sct_title FROM '.$this->db->dbprefix('sct').' AS sct LEFT JOIN '.$this->db->dbprefix('sct_trl').' AS sct_trl ON sct_trl.sct_id = sct.sct_id WHERE sct_trl.lng_id = \''.$this->lng[0]->lng_id.'\' GROUP BY sct.sct_id ORDER BY sct.sct_code');
		foreach ($query->result() as $row) {
    ...
    function select_language() {
		...
		$select_language[''] = '--';
        $query = $this->db->query('SELECT lng.lng_id, CONCAT(lng.lng_title, \' (\', lng.lng_code, \')\') AS lng_title FROM '.$this->db->dbprefix('lng').' AS lng WHERE 1 GROUP BY lng.lng_id ORDER BY lng.lng_code');
		foreach ($query->result() as $row) {
